...
const Product = () => {
  ...
  const GetProduct = () => {
    axios
      .get(`https://api-strawberrynet.herokuapp.com/products/${id}`)
      .then((res) => {
  ...
  const addProduct = {
    ...
    size: [
      {
        size: product.size ? product.size[size].size : "",
        price: product.size ? product.size[size].price : "0",
      },
    ],
    qty: Number(qty),
  };
  ...
  const addtoBag = () => {
    if (!user) {
      ...
      const isPresent =
        bag.length > 0 &&
        bag.map((el) =>
          el._id === addProduct._id
            ? { ...el, qty: Number(el.qty) + Number(qty) }
            : el
        );
      // const isPresent= bag.length>0&& bag.find((el)=> el.id===product.id)
      const isPresentObject =
        bag.length > 0 &&
        bag.filter(
          (el) =>
            el._id === addProduct._id && {
              ...el,
              qty: Number(el.qty) + Number(qty),
            }
        );
              ...
              <div className={styles.sale_card_div}>
                {product.size.map((obj, i) => (
                  <div
                    onClick={() => setSize(i)}
                    className={
                      size === i ? styles.active_sale_card : styles.sale_card
                    }
                  >
                    <div>sale</div>
                    <div>{obj.size}</div>
              ...
              <div>
                <div className={styles.tabs}>
                  <div
                    ...
                    className={activeTab === "details" && styles.active_tab}
                  >
                    <p style={activeDetails}>PRODUCT DETAILS</p>
                  </div>
                  <div
                    onClick={() => setActiveTab("review")}
                    className={activeTab === "review" && styles.active_tab}
                  >
                    <p style={activeReview}>
                  ...
                  <div>
                    <div className={styles.wish_list}>
                      {" "}
                      <FavoriteBorderIcon className={styles.wish_icon} />
                      <p>ADD TO WISHLIST</p>
                    </div>
                  </div>
